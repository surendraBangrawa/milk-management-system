...
class AddMilkRecordRequest(BaseModel):
    ...
    shift: str = Field(
        None, pattern="^(M|E)$"
    )  # ✅ Optional, auto-populated if missing
...
def add_milk_record(
    ...
    try:
        ...
        total_till_record = round(
            total_till_previous + (quantity * rate), 2
        )  # ✅ Round total value
...
def add_expense(
    ...
    try:
        ...
        if record.custom_date is None:
            ...
            record.custom_date = datetime.combine(
                record.custom_date, local_time.time()
            )  # ✅ Merge user date with current time
...
def update_total_till_records(
    ...
    future_milk_records = (
        ...
        .order_by(MilkRecord.added_at.asc())  # Then by added_at for final tie-breaker
        .all()
    )
...
def edit_transaction(
    ...
    try:
        logger.info(f"In edit transaction")
        local_time = datetime.now(local_timezone).replace(tzinfo=None)
        ...
        )  # ✅ Difference to be adjusted in future records
...
def get_seller_summary(
    ...
    try:
        logger.info(f"In get_seller_summary")
        seller_details = []
...
def get_Buyer_summary(
    ...
    try:
        logger.info(f"In get_buyer_summary")
        buyer_details = []
...
class EditProfile(BaseModel):
    seller_mobile: Optional[str] = Field(
        None, pattern="^[0-9]{10}$"
    )  # Optional seller_mobile
    new_name: str
...
def edit_profile(
    ...
    try:
        ...
        # if seller number is not given then get own record
            ...
            record_to_change = (
                old_own_record  # Set the record to change to the own record
            )
...
def delete_customer(
    ...
    try:
        logger.info(f"In delete_customer")
...
def delete_account(
    ...
    try:
        logger.info(f"In delete_account")
        account_record = (
            db.query(User)
            .filter(User.mobile == buyer_mobile, User.is_deleted == 0)
            .first()
...
def store_rate_list(
    ...
    try:
        logger.info(f"In store_rate_list")
        get_user = (
            db.query(User)
            .filter(User.mobile == buyer_mobile, User.is_deleted == 0)
            .first()
...
def delete_rate_list(
    ...
    try:
        logger.info(f"In delete_rate_list")
        rate_list_record = (
            db.query(RateList)
            .filter(RateList.buyer_mobile == buyer_mobile, RateList.is_deleted == 0)
            .first()
