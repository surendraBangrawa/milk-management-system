...
def signup(user: SignupRequest, db: Session = Depends(get_db)):
    ...
    if existing_user:
        if existing_user.is_deleted == 1:
            ...
            raise HTTPException(
                status_code=400, detail="User with this mobile number already exists"
            )
...
def send_login_otp(user: OtpRequest, db: Session = Depends(get_db)):
    ...
    existing_user = (
        db.query(User).filter(User.mobile == user.mobile, User.is_deleted == 0).first()
    )
...
def add_customer(
    ...
    existing_customer = (
        db.query(Customer)
        .filter(
            Customer.mobile == customer.mobile, Customer.added_under == current_mobile
        )
        .first()
    )
...
def edit_profile(
    ...
    if record.seller_mobile:
        ...
        record_to_change = (
            old_customer_record  # Set the record to change to the customers's record
        )
