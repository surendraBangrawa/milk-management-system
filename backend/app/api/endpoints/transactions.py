...
def update_total_till_records(
    ...
    """
    Update total_till_record for all records after the given date.
    This function recalculates the running balance properly.
    """
    try:
        # Get all records for this buyer-seller pair, ordered by added_at
        all_milk_records = (
            db.query(MilkRecord)
            .filter(
        ...
        # Combine and sort all records by added_at
        all_records = sorted(
            all_milk_records + all_expense_records,
            key=lambda record: record.added_at,
        )

        # Recalculate running balance for all records
        running_balance = 0.0
        ...
        db.commit()
        logger.info(
            f"Successfully updated total_till_record for {len(all_records)} records"
        )

    except Exception as e:
        db.rollback()
        logger.error(f"Error updating total_till_records: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to update transaction balances"
        )
...
def add_milk_record(
    ...
    try:
        ...
        # Check usage limits before adding transaction
        allowed, message = subscription_service.check_usage_limits(
            db, buyer_mobile, "add_transaction"
        )
        if not allowed:
        ...
        subscription_service.update_usage(db, buyer_mobile, "add_transaction")
